...
start = time.time()   
initialization()
numberOfConstraint1_f = open("constraints_1.txt", "r") 
numberOfConstraint2_f = open("constraints_2.txt", "r")
numberOfConstraint1 = int(numberOfConstraint1_f.read())
numberOfConstraint2 = int(numberOfConstraint2_f.read())
print(numberOfConstraint1)
print(numberOfConstraint2)


constraints_1 = []
for i in range (numberOfConstraint1):
    infile = open('constraints_1'+str(i)+'.pkl','rb')
    constraints_1.append(pickle.load(infile))
    infile.close()

constraints_2 = []
for i in range (numberOfConstraint2):
    infile = open('constraints_2'+str(i)+'.pkl','rb')
    constraints_2.append(pickle.load(infile))
    infile.close()
...
#print(type(constraints_2[0]))
#print(type(constraints_2))
notEquivalent=True
for i in range (numberOfConstraint1):
    #print("===", c1)
    for j in range (len(constraints_2)):
        #print("i ",i," j ", j )
        c1 = constraints_1[i]
        c2 = constraints_2[j]
        #print("+++", c2)
        solver = claripy.Solver()
        solver.add(claripy.Not(claripy.Or(claripy.And(c1, c2), claripy.And(claripy.Not(c1), claripy.Not(c2)))))
        notEquivalent = solver.satisfiable()
        if(notEquivalent==False):
            #print(c1, "and ",c2)
            f1.write(str(c1))
            f1.write("\n")
            f1.write(str(c2))
